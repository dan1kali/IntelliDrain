...
void setup() {
  ...
  if (Sensor_LoadCell.getTareTimeoutFlag() || Drainage_WeightScale.getTareTimeoutFlag() || Saline_WeightScale.getTareTimeoutFlag()) {
    if (Sensor_LoadCell.getTareTimeoutFlag()){
      Serial.println("load cell timeout");
    }
    if (Drainage_WeightScale.getTareTimeoutFlag()){
      Serial.println("Drainage weight scale timeout");
    }
    if (Saline_WeightScale.getTareTimeoutFlag()){
      Serial.println("Saline weight scale timeout");
    }
    Serial.println("Timeout, check load cell and weight scale wiring and pin designations");
    while (1);
  }

  // if (Sensor_LoadCell.getTareTimeoutFlag() || Drainage_WeightScale.getTareTimeoutFlag()) {
  //   if (Sensor_LoadCell.getTareTimeoutFlag()){
  //     Serial.println("load cell timeout");
  //   }
  //   if (Drainage_WeightScale.getTareTimeoutFlag()){
...
void loop() {
  ...
  const int stabilizationRequired = 5; // Number of good readings needed
