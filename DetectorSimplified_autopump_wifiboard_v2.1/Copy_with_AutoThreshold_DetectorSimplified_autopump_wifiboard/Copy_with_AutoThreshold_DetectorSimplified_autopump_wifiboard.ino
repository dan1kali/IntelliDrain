...
void setup() {
  ...
  Serial.println(openAverage); // DELETE LATER
  int closedAverage = closed.getAvg();
  Serial.print("Closed average: ");
  Serial.println(closedAverage); // DELETE LATER
  threshold = 0.5 * (openAverage + closedAverage); // threshold is average between open and closed states
  ...
  float weighttest = LoadCell_0.getData();
  Serial.print("TEST:");
  Serial.println(weighttest);
